import { MessageSquare, Server, Terminal } from "lucide-react";

import { botCommands, categoryOrder } from "@/lib/commands";
import { cn } from "@/lib/utils";

const stats = [
  { label: "Slash Commands", value: botCommands.length.toString(), icon: Terminal },
  { label: "Categories", value: categoryOrder.length.toString(), icon: Server },
  { label: "Active Development", value: "v2.7.0", icon: MessageSquare },
];

export function StatsBar() {
  return (
    <section className="relative border-y border-white/[0.08] bg-[#020815]">
      <div className="absolute inset-0 bg-gradient-to-r from-atsuko-cyan/[0.03] via-transparent to-atsuko-pink/[0.03]" />
      <div className="relative mx-auto grid max-w-[1450px] gap-0 px-5 py-6 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-4 sm:grid-cols-3 sm:px-8">
        {stats.map((stat, index) => {
          const Icon = stat.icon;
          return (
            <div
              key={stat.label}
              className={cn(
                "flex items-center justify-center gap-3 px-4 py-3 text-center",
                index > 0 && "sm:border-l sm:border-white/[0.08]",
              )}
            >
              <Icon className="size-5 text-atsuko-cyan" />
              <div className="flex flex-col items-center">
                <p className="text-2xl font-extrabold text-white">{stat.value}</p>
                <p className="text-xs font-medium text-slate-400">{stat.label}</p>
              </div>
            </div>
          );
        })}
      </div>
    </section>
  );
}
